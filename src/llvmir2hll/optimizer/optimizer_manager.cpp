...
void CopyPropagationOptimizer::runOnFunction(ShPtr<Function> func) {
	...
	//TODO 这种改法比较失败。
	// if (func->getTextRepr() == "def LBM_swapGrids1(**grid1, **grid2)"){
...
//判断两个语句之间是否存在循环语句
bool isUseinWhileOrForLoop(ShPtr<Statement> stmt,ShPtr<Statement> use){

	while(stmt->hasSuccessor() &&stmt->getSuccessor() != use){
		if (isLoop(stmt->getSuccessor()))
			return true;
...
//判断该语句与其之后的使用语句之间是否存在着while 或者 for循环
bool isUsesinWhileOrForLoop(ShPtr<Statement> stmt,const StmtSet &uses){
	for (const auto &use : uses) {
		if (isUseinWhileOrForLoop(stmt,use)) {
			return true;
		}
	}
	return false;
...
//只要保留addr ，在交换的情况下
bool TheSameStmt(ShPtr<Statement> stmt){
	if (isa<AssignStmt>(stmt)){
		if (getLhs(stmt)->getTextRepr().find("addr") != std::string::npos || getRhs(stmt)->getTextRepr().find("addr") != std::string::npos){
			return true;
		}				
	}
	return false;	
...
//判断该左变量是否在使用的右值上出现
// bool isStmtLhsVarInRight(ShPtr<Variable> stmtLhsVar,const StmtSet &uses){
// 	for (const auto &use : uses) {
// 		if (stmtLhsVar == getRhs(use)) {
// 			return true;
// 		}
// 	}
// 	return false;
...
void CopyPropagationOptimizer::performOptimization() {
	...
	for (const auto &du : ordered(ducs->du)) {
		...
		if (endsWithRetOrUnreach(stmt)){
			//std::cout<<stmt->getTextRepr()<<std::endl;
			LOG << "\t" << "end 1" << std::endl;
			continue;
		}

		if (isa<WhileLoopStmt>(stmt->getSuccessor())) {
			// std::cout<<stmt->getTextRepr()<<std::endl;
			LOG << "\t" << "end 1" << std::endl;
			MAX_STMT_LENGTH = 35;
			continue;
		}
		//cq add TODO 这些代码仍然需要修正
		// if (stmt->getTextRepr().find("__this_group_max = ")!= std::string::npos){
		// 	continue;
		// }
		
		// if (stmt->getTextRepr().find("__cl_parm_kGlobalIndexe =")!= std::string::npos){
		// 	continue;
		// }
		...
		//当 赋值语句中，左值在后续loop中做了右值，并且右值在后续loop中做了左值，那么就不应该优化
		// int a;
		// int b;
		// a = b;
		// ...
		...
		// 	int d = b;
		// }
		// if (stmt->getTextRepr().find("blocks_work_size =")!= std::string::npos){
			
		// }
		//TODO 目前做不到精确定位，只能粗略定位当使用与定义中间隔着循环，就不再优化。
		if(isUsesinWhileOrForLoop(stmt,uses))
			// if(isStmtLhsVarInRight(getLhs(stmt),uses))
			// 	if(isStmtRhsVarInLeft(getRhs(stmt),))
					continue;

		//cq ++
		// 当出现了
		// a b a b 这四句语句时，就不再进行优化
		if (TheSameStmt(stmt))
			continue;
