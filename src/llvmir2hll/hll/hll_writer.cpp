...
*/
#include <array>
// std::vector<std::string> multiple_definition {"cl_errChk","cl_cleanup","coo_to_jds","sort_stats","sort_rows","mm_typecode_to_str",
// 												"mm_strdup","mm_read_mtx_crd_size","mm_read_banner","clMemSet","readFile","kmeans_clustering",
// 												"mm_is_valid","mm_read_mtx_crd_data","oclDebugErrString","outputData","inputData","oclErrorString",
// 												"cl_getPlatformVendor","create_bmp","HSVtoRGB","dump_histo_img","oclLoadProgSource",
// 												"getOpenCLDevice","getOpenCLPlatform","checkDeviceReq","gettime"};
...
bool HLLWriter::emitFunctions() {
	...
	for (const auto &func : funcs) {
		...
		//if (func->getName().compare("cl_errChk") == 0 || func->getName().compare("cl_cleanup") == 0)
		// if (std::find(multiple_definition.begin(), multiple_definition.end(), func->getName()) != multiple_definition.end())
		// 	continue;
...
void HLLWriter::emitBinaryOpExpr(const std::string &opRepr,
	...
	bool bracketsAreNeeded = bracketsManager->areBracketsNeeded(expr);
	// 当式子是 X = A * (B/C) ;时，且C为整数时，不能够 删除括号，否则值会不对
	if (opRepr.compare("/") == 0 && isa<IntType>(expr->getSecondOperand()->getType()))
		bracketsAreNeeded = true;
	if (bracketsAreNeeded) {
		out->punctuation('(');
	}
